# BetterBanners

Flag banners and oversized banner tiers.

<!-- img: flag banners on posts -->



## Features

- **Flag banners** - mount any banner as a hanging flag on fence posts, chains, and bars.
- **Large & Huge banners** - upgrade normal banners into bigger variants that keep their patterns. See [the catalog](https://def9a2a4.github.io/defCoreLib-docs/index.html/index.html?q=banner).
- Large/Huge banners are the ingredient for [Mechanism](https://github.com/def9a2a4/defCoreLib/blob/main/docs/readmes/mech.md) Large and Huge windmills.

## Requires

[DefCoreLib](https://github.com/def9a2a4/defCoreLib/blob/main/docs/readmes/defCoreLib.md).

## Links

- Recipes & details: https://def9a2a4.github.io/defCoreLib-docs/?q=banner
- Repository: https://github.com/def9a2a4/defCoreLib/
- Issues: https://github.com/def9a2a4/defCoreLib/issues
